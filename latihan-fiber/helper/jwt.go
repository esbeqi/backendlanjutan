...
func (m *JWTManager) Parse(
	...
	token, err := jwt.ParseWithClaims(
		...
		func(t *jwt.Token) (any, error) {
			...
			// PEMERIKSAAN WAJIB: pastikan algoritmanya memang
			// yang kita pilih.
			// Tanpa baris ini, penyerang dapat mengirim token
			// ber-alg "none" atau menukar algoritma.
			if _, ok := t.Method.(*jwt.SigningMethodHMAC); !ok {
				return nil, fmt.Errorf(
					"algoritma tidak diharapkan: %v",
					t.Header["alg"],
				)
			}
